...
class ExtraZeroesTest(unittest.TestCase):
    def setUp(self):
        # Key, ciphertext, and plaintext taken from https://github.com/sybrenstuvel/python-rsa/issues/146
    ...
    def test_unmodified(self):
        message = rsa.decrypt(self.ciphertext, self.private_key)
        self.assertEqual(message, self.plaintext)

    def test_prepend_zeroes(self):
        ciphertext = bytes.fromhex("0000") + self.ciphertext
        with self.assertRaises(rsa.DecryptionError):
            rsa.decrypt(ciphertext, self.private_key)

    def test_append_zeroes(self):
        ciphertext = self.ciphertext + bytes.fromhex("0000")
        with self.assertRaises(rsa.DecryptionError):
            rsa.decrypt(ciphertext, self.private_key)
...
-----END RSA PRIVATE KEY-----
        ...
        ciphertext = self.encrypt_with_short_padding(b"op je hoofd")
        with self.assertRaises(rsa.DecryptionError):
            rsa.decrypt(ciphertext, self.private_key)
    ...
    def encrypt_with_short_padding(self, message: bytes) -> bytes:
        ...
        ciphertext = rsa.transform.int2bytes(encrypted_value, keylength)
